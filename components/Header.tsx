            ...
            <>
                <header className='bg-[#f8f8f8] dark:bg-[#06070D] flex fixed top-0 left-0 right-0 lg:h-20 h-auto z-[100] items-center lg:flex-row flex-col shadow-black drop-shadow-xl'>
                    <span className='flex items-center lg:mx-10 lg:my-auto my-4 text-gray-900 dark:text-gray-200 font-bold cursor-pointer'>
                        <Link href="/" passHref>
                            <svg aria-label="Matrix Art Logo" id="matrix-art-logo" width="8.75rem" className="fill-gray-900 dark:fill-gray-200" viewBox="0 0 400 82" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className='flex lg:flex-1 items-center justify-between lg:flex-row flex-col'>
                        <div className='flex grow items-center'>
                            <form className='w-52 text-gray-900 dark:text-gray-200'>
                                <div className='flex flex-row box-border items-center cursor-text duration-300 rounded-sm border dark:border-slate-400 border-slate-500 py-1.5 px-2 focus-within:border-teal-400'>
                                    <input className='bg-transparent min-w-[1.25rem] focus:outline-none flex-[1] border-none' type="text" placeholder={i18n?.t('Search & Discover')} />
                        ...
                        <nav className='flex lg:flex-shrink-0 my-4'>
                            {!this.state.loggedIn ? <span className='px-4 h-auto min-w-[1.5rem] flex items-center whitespace-nowrap cursor-pointer text-gray-900 dark:text-gray-200 font-medium'><Link href="/register">{i18n?.t('Join')}</Link></span> : undefined}
                            {!this.state.loggedIn ? <span className='px-4 h-auto min-w-[1.5rem] flex items-center whitespace-nowrap cursor-pointer text-gray-900 dark:text-gray-200 font-medium'><Link href="/login">{i18n?.t('Log in')}</Link></span> : undefined}
                            {this.state.loggedIn && this.state.directory_data.some(thing => thing.mxid == this.context.client.userId) ? <span className='px-4 h-auto min-w-[1.5rem] flex items-center whitespace-nowrap cursor-pointer text-gray-900 dark:text-gray-200 font-medium'><Link href={"/profile/" + encodeURIComponent(this.context.client.userId!)}>{i18n?.t('Profile')}</Link></span> : undefined}
                            {this.state.loggedIn ? <span className='px-4 h-auto min-w-[1.5rem] flex items-center whitespace-nowrap cursor-pointer text-gray-900 dark:text-gray-200 font-medium'><Link href="/logout/">{i18n?.t('Logout')}</Link></span> : undefined}
                        </nav>
                    </div>
